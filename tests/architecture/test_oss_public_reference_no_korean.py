...
Per ``docs/impl/549`` D5, the v1.0 rendered reference pages
...
``baldur.adapters.memory``. ``docs/impl/548`` SC #1 used a top-level-only glob
...
path list, which would repeat 548's blind spot (R3).

Code comments/docstrings/log messages must be English per CLAUDE.md
§ Code Language Rules. Baseline is enforced-empty.

Rule registry: ``docs/laws/ARCHITECTURAL_FITNESS_FUNCTIONS.md#g23-oss-reference-no-korean``
"""
