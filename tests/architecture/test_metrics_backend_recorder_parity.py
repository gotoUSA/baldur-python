...
fail-open ``try/except`` swallowed. The defect is the ADR-008
behaviorally-invisible class — a dead recording path changes no test output — so
...
Rule registry:
``docs/laws/ARCHITECTURAL_FITNESS_FUNCTIONS.md#g46-metrics-backend-recorder-parity``
"""
...
class TestMetricsBackendHistogramBucketParity:
    ...
    Rule registry:
    ``docs/laws/ARCHITECTURAL_FITNESS_FUNCTIONS.md#g47-metrics-backend-histogram-bucket-parity``
    """
