"""G7 — ADR-006 startup wiring (`setup_*` / `start_*` MUST be invoked).

Per CLAUDE.md § Pattern Compliance — Startup wiring: every public
``setup_*()`` / ``start_*()`` function MUST have a call site in a framework
adapter's startup path. Defined-but-uncalled setup functions are bugs (ADR-006
Consequences).
...
Rule registry: ``docs/laws/ARCHITECTURAL_FITNESS_FUNCTIONS.md#g7-startup-wiring``
"""
